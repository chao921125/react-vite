...
export const setRouter = () => {
	rootRouter = [...rootRouter, ...routeToComponent(RouterData.menus)];
	baseRouters[0].children = [...rootRouter];
	console.log(baseRouters);
	routes = [...errorRouters, ...baseRouters];
	Routers = () => useRoutes(generateRouter(routes));
};

export function routeToComponent(routes: any[]) {
	if (!routes) return [];
	return routes.map((item: any) => {
		if (item.component) {
			item.path = `/${item.path}`;
			item.component = componentImport(item.component as string);
		}
...
function componentImport(component: string) {
